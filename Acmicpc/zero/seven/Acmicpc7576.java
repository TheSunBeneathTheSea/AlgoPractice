...
public class Acmicpc7576 {
  private static int[] rowMove = new int[]{1, -1, 0, 0};
  private static int[] colMove = new int[]{0, 0, 1, -1};
  ...
  public static int solve(int[][] map) {
    ...
    while (!queue.isEmpty()) {
      ...
      for (int i = 0; i < 4; i++) {
        Integer[] next = new Integer[]{cur[0] + rowMove[i], cur[1] + colMove[i]};

        // 다음 좌표 타당성 체크
        if (next[0] < 0 || next[0] >= map.length || next[1] < 0 || next[1] >= map[0].length
                || map[next[0]][next[1]] < 0 || isVisited[next[0]][next[1]]) {
          continue;
        }
