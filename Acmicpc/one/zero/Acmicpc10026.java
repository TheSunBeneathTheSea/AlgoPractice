      ...
      for (int j = 0; j < map.length; j++) {
        if (!isVisited[i][j]) {
          ...
          while (!queue.isEmpty()) {
            ...
            for (int k = 0; k < 4; k++) {
              Integer[] next = new Integer[]{cur[0] + colMove[k], cur[1] + rowMove[k]};

              if (next[0] < 0 || next[0] >= map.length || next[1] < 0 || next[1] >= map[0].length
                      || isVisited[next[0]][next[1]]) {
                continue;
              }
              if (map[next[0]][next[1]] == target) {
                queue.add(next);
        ...
        if (!isVisited[i][j]) {
          ...
          while (!queue.isEmpty()) {
            ...
            for (int k = 0; k < 4; k++) {
              ...
              char nextTarget = map[next[0]][next[1]];
              if (nextTarget == target || (target != 'B' && nextTarget != 'B')) {
                queue.add(next);
                isVisited[next[0]][next[1]] = true;
              }
            }
